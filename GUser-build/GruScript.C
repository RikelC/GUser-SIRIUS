...
void perform(){
	...
	configFile.open("ConfigFiles/Run.config", std::ifstream::in);
		...
		for(unsigned int l =0; l < runfiles.size(); l++){
			...
			if(sum_runs == "YES" && runfiles.size() > 1){
				if(l ==0){
					...
					a= new GUser(file);            // creat user treatement environement
					a->g_instance = a;
					...
					if(save_ttree == "YES")	a->SetTTreeMode(3,treeFileName.c_str());
	...
	for(unsigned int i =0; i < list.size(); i++){
		...
		if(found!= std::string::npos){
			...
			if(r_length == l_length){
				...
				srange[1] = extract_subrun_number_from_filename(list[i]);
			}
		}
	}
	return srange;
